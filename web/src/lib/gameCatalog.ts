...
import { type GameSectionId, gameSections } from '../i18n/gameSections'

const GAME_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../../game')
...
function isPublicMeta(data: Record<string, unknown>): boolean {
  ...
  return !(spoilers === 'gm' || spoilers === 'secret')
}
...
export function gameItemsInSection(sectionId: GameSectionId): GameItem[] {
  return loadGameCatalog().filter((index) => index.sectionId === sectionId)
}

export function findGameItem(sectionId: string, slug: string): GameItem | undefined {
  return loadGameCatalog().find((index) => index.sectionId === sectionId && index.slug === slug)
}
