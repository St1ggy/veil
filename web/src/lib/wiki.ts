...
import { type CategoryId, categoryForType } from '../i18n/categories'
...
export function categoryHref(lang: Lang, categoryId: CategoryId | string): string {
  ...
  if (categoryId === 'worldview') return withBase(`${lang}/worldview`)
...
export function gameItemHref(lang: Lang, sectionId: string, slug: string): string {
...
export async function getWikiEntries(lang: Lang = 'ru'): Promise<WikiEntry[]> {
  ...
  return entries
    ...
    .filter((e) => !e.id.includes('_templates'))
