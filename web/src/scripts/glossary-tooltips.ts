...
type TooltipTarget = HTMLElement & {
  dataset: DOMStringMap & { tooltip?: string; tooltipKind?: string; tooltipTitle?: string; href?: string }
}
...
;(() => {
  ...
  activateTerms(
    main,
    automaticSwadeTerms.map(([name, original]) => ({
      name,
      definition: `[${original}][${swadeBook}]`,
      normalized: name.replaceAll('ё', 'е'),
      kind: 'swade' as const,
    })),
  )
    ...
    .then((terms) => {
      activateTerms(
        ...
        terms
          ...
          .map((term) => ({
            ...term,
            normalized: term.name.replaceAll('ё', 'е'),
  ...
  popup.hidden = true
  popup.innerHTML =
    '<span class="rich-tooltip__kind"></span><strong class="rich-tooltip__term"></strong><span class="rich-tooltip__body"></span>'
  ...
  const show = (target: TooltipTarget) => {
    ...
    kind.textContent =
      target.dataset.tooltipKind === 'swade'
        ? 'Термин «Дневника авантюриста»'
        : target.dataset.tooltipKind === 'control'
          ? 'Фильтр мировоззрения'
          : 'Глоссарий «Вуали Миров»'
    term.textContent = target.dataset.tooltipTitle ?? target.textContent?.trim() ?? ''
